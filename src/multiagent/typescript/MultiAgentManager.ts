...
import { MultiAgentEnvironment, AgentObservation, AgentAction } from './MultiAgentEnvironment';
import logger from '../../utils/logger';
import { DefaultAgentFactory } from '../../utils/DefaultAgentFactory';
...
export class MultiAgentManager {
  ...
  constructor(config: MultiAgentManagerConfig) {
    ...
    // Set agent factory
    this.agentFactory = config.agentFactory || new DefaultAgentFactory();
  ...
  public resetAgents(): boolean {
    try {
      ...
      this.logger.debug(`Reset ${this.agents.size} agents for new episode`);
      
      return true;
    } catch (error) {
      this.logger.error(`Error resetting agents: ${(error as Error).message}`);
      return false;
    }
  }
   ...
   */
  public terminateAgents(): boolean {
    try {
      for (const wrapper of this.agents.values()) {
        try {
          wrapper.agent.terminate();
        } catch (error) {
          this.logger.warn(`Error terminating agent ${wrapper.id}: ${(error as Error).message}`);
        }
      }
      
      this.logger.info(`Terminated ${this.agents.size} agents`);
      
      return true;
    } catch (error) {
      this.logger.error(`Error terminating agents: ${(error as Error).message}`);
      return false;
    }
  }
   ...
   * @param teamId Team ID
   * @returns Array of agent wrappers in the team
   */
  public getTeamAgents(teamId: string): AgentWrapper[] {
   ...
   * @param agentId Agent ID
   * @param teamId Team ID
   * @returns True if agent was added to team, false otherwise
   */
  public addAgentToTeam(agentId: string, teamId: string): boolean {
    // Check if agent exists
    const wrapper = this.agents.get(agentId);
    if (!wrapper) {
      this.logger.warn(`Agent with ID ${agentId} does not exist`);
    ...
    if (team.agentIds.includes(agentId)) {
      this.logger.warn(`Agent ${agentId} is already in team ${teamId}`);
      return false;
    }
    ...
    this.logger.info(`Added agent ${agentId} to team ${teamId}`);
   ...
   * @param agentId Agent ID
   * @param teamId Team ID
   * @returns True if agent was removed from team, false otherwise
   */
  public removeAgentFromTeam(agentId: string, teamId: string): boolean {
    // Check if agent exists
    const wrapper = this.agents.get(agentId);
    if (!wrapper) {
      this.logger.warn(`Agent with ID ${agentId} does not exist`);
    ...
    const agentIndex = team.agentIds.indexOf(agentId);
    if (agentIndex === -1) {
      this.logger.warn(`Agent ${agentId} is not in team ${teamId}`);
      return false;
    }
