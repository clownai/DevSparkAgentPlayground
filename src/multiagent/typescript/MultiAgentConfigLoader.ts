...
export class MultiAgentConfigLoader {
  ...
  public validateExperimentConfig(config: MultiAgentExperimentConfig): { valid: boolean, errors: string[] } {
    ...
    // Check turn order if action mode is turn-based
    if (config.actionMode === 'turn-based' && (!config.turnOrder || config.turnOrder.length === 0)) {
      errors.push('Turn-based action mode requires a non-empty turnOrder array');
    }
    
    // Check max steps and episodes
    if (typeof config.maxSteps !== 'number' || config.maxSteps <= 0) {
      errors.push(`Invalid maxSteps: ${config.maxSteps}, expected positive number`);
    }
    
    if (typeof config.maxEpisodes !== 'number' || config.maxEpisodes <= 0) {
      errors.push(`Invalid maxEpisodes: ${config.maxEpisodes}, expected positive number`);
    }
